...
namespace CEMIS.Web.CentralPortal
{
    ...
    public class UserController : BaseController
    {
        ...
        public UserController(UserManager<User> userManager, IUserManagementServices userService, IRoleService roleService,
            RoleManager<Role> roleManager, IPasswordService passwordService, IMessageManagementService messageService, IMailManagementService mailService,
            ILogger<UserController> logger, SignInManager<User> signInManager, IConfiguration configuration, IHttpContextAccessor accessor, IActivityLog logActivityRepo,
        ...
        public async Task<IActionResult> AddUser([Bind("UserName,LastName,FirstName,Email,RoleId")]UsersObject model)
        {
            var roles = _roleManager.Roles.Select(x =>
             new SelectListItem
             {
            ...
            var firstnamevalue = firstname.Substring(0);
            ...
            try { 
               ...
               .Select(p => p.UserName);
                if (userId.Contains(model.UserName.ToLower().Trim()))
                {
                    ...
                    //this.AddNotification($"Username already exist on the system!", NotificationType.WARNING);
                ...
                var appuser = new User { Email = model.Email, UserName = model.UserName.ToLower(), EmailConfirmed = false, IsActive = true, TwoFactorEnabled = false, PwdExpiryDate = x30MinsLater, Lastname = model.LastName, Middlename = model.MiddleName, Firstname = model.FirstName , UserType = (byte)UserType.Admin};
                    ...
                    if (userrole.Succeeded)
                    {
                       ...
                       // passwordModel.CreatedBy = curr
                        ...
                        if (mail == 1001)
                        {
                            ...
                            await LogActivity("User created successfully", model);
                            return RedirectToAction(nameof(Users));
                        }

                        TempData["Notification"] = JsonConvert.SerializeObject(new Notification { Message = "User created successfully", Title = "User Management", Type =  NotificationsType.Success });
            ...
            catch(Exception ex)
            {
                ...
                TempData["Notification"] = JsonConvert.SerializeObject(new Notification { Message = "Operation failed", Title = "User Management", Type = NotificationsType.Danger });
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Userupdate([Bind("UserName,LastName,FirstName,AppUserId,Email,RoleId")]UsersObject model)
        {
            ...
            try { 
                ...
                if (identityupdate.Succeeded)
                {
                    ...
                    if (update)
                    {
                        TempData["Notification"] = new Notification { Message = "profile was successfully updated!", Title = "User Account", Type = NotificationsType.Success };
                        await LogActivity("profile was successfully updated!", model);
                        return RedirectToAction(nameof(Users));
                    }
                ...
                TempData["Notification"] = new Notification { Message = "profile was not successfully updated!", Title = "User Account", Type = NotificationsType.Warning };
                await LogActivity("profile was not successfully updated!", model);
               return RedirectToAction(nameof(Users));
        ...
        public async Task<IActionResult> Activate_user([Bind("AppUserId")] UsersObject model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (activate)
                {
                    await LogActivity("User activated Succefully", model);
                    return RedirectToAction(nameof(Users));
                }
                await LogActivity("User not activated successfully", model);
        ...
        {
           var res = _userService.CheckEmailValidity(email);
           return Json(res);
        }
        ...
        public async Task<IActionResult> Deactivate_user([Bind("AppUserId")] UsersObject model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (User.Identity.IsAuthenticated && User.Identity.Name == getuser.UserName)
                {
                    ...
                    TempData["Notification"] = new Notification { Message = "Sorry you cannot deactivate your own account! Thanks", Title = "User Account", Type = NotificationsType.Danger };
                    await LogActivity("You can't deactivate ypur own account", model);
                    return RedirectToAction(nameof(Users));
                ...
                if (deactivate)
                {
                    ...
                    TempData["Notification"] = new Notification { Message = "User deactivated successfully", Title = "User Account", Type = NotificationsType.Danger };
                    await LogActivity("User deactivated successfully", model);
                    return RedirectToAction(nameof(Users));
                }
            }
            return View(model);
